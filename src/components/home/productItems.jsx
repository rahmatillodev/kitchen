...
export default function ProductItems({ products, onAdd }) {
  return (
    <div className="w-full h-full pt-4">
      <div className="grid grid-cols-5 gap-4 overflow-x-auto pb-4 text-center mb-6">
        {products.map(category => (
          <span key={category.id} className="font-semibold text-base whitespace-nowrap">
            {category.name}
          </span>
        ))}
      </div>
      ...
      {products.map(category => (
        <div key={category.id} className="mb-8 border-r-2 p-1">
